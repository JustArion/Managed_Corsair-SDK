...
public struct LedController
{
    internal LedController(in CorsairDeviceInfo device)
    {
        ...
        _deviceInfo = device;
        var ledInfo = TryGetLedInformation();
        if (!ledInfo.Success)
        {
            _ledInformation = Array.Empty<LedInformation>();
            return;
        }

        _ledInformation = ledInfo.LedInformation;
    }

    private readonly LedInformation[] _ledInformation;
    ...
    {
        var (success, ledInformation) = TryGetLedInformation();
        if (!success)
            return false;
    ...
    public async Task PulseSingleKey(CorsairLedColor from, CorsairLedColor to, TimeSpan animationDuration, bool isInfinite)
    {
        if (from.id != to.id)
        {
            throw new InvalidOperationException("The animation targets should have the same length");
        ...
        while (Environment.TickCount - startTime < animationDuration.TotalMilliseconds)
        {
            var progress = (Environment.TickCount - startTime) / animationDuration.TotalMilliseconds;

            var lerpedColor = InterpolateColor(from, to, progress);
