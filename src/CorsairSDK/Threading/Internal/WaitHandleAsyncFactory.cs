...
// Credit goes to https://github.com/StephenCleary
// https://github.com/StephenCleary/AsyncEx/blob/master/src/Nito.AsyncEx.Interop.WaitHandles/Interop/WaitHandleAsyncFactory.cs (MIT)
internal static class WaitHandleAsyncFactory
{
    ...
    public static Task<bool> FromWaitHandle(WaitHandle handle, TimeSpan timeout, CancellationToken token)
    {
        ...
        return token.IsCancellationRequested
            ...
            : DoFromWaitHandle(handle, timeout, token);  // Register all asynchronous cases.
    }
    ...
    private static async Task<bool> DoFromWaitHandle(WaitHandle handle, TimeSpan timeout, CancellationToken token)
    {
        ...
        await using (token.Register(state => ((TaskCompletionSource<bool>) state!).TrySetCanceled(), tcs, useSynchronizationContext: false))
